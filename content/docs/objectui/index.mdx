...
4. **Renderer Interface**: Standardized contract for concepts implementations
...
**Concepts Agnostic**: The protocol never assumes React, Vue, or any specific concepts. Renderers translate protocols to concepts-specific implementations.
---
...
3. **Component Resolution**: Maps protocol types to concepts components
4. **Data Binding**: Connects components to ObjectQL data sources
5. **Rendering**: Concepts-specific components rendered
...
This architecture ensures **complete concepts independence**—changing from React to Vue requires zero protocol changes.
...
ObjectUI defines a **standardized renderer contract** that all concepts implementations must support:
...
- **Concepts Agnostic**: Same protocol renders in React, Vue, Flutter
...
- **[The Manifesto](/docs/concepts/manifesto)** - Why ObjectUI is protocol-driven and concepts-agnostic
- **[Architecture](/docs/concepts/architecture)** - How ObjectUI fits into the ObjectStack ecosystem
